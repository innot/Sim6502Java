...
public class Sim6520 {
    ...
    private void update_output() {
        ...
        // the output register.
        if ((this.cra & CR_IRQ1_ENABLE) != 0 ||
                ((this.cra & CRA_CAB2_MODE) == 0 && (this.cra & CR_IRQ2_ENABLE) != 0)) {
            int irq = (this.cra & CR_IRQ_1_FLAG) | (this.cra & CR_IRQ_2_FLAG);
            output.irqa = irq == 0; // active low
        }

        //
        if ((this.crb & CR_IRQ1_ENABLE) != 0 ||
                ((this.crb & CRA_CAB2_MODE) == 0 && (this.crb & CR_IRQ2_ENABLE) != 0)) {
