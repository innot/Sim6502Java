...
public class Sim6522 {
    ...
    Sim6522Output output = new Sim6522Output();
    ...
    private void init_timer(TimerState t, boolean is_reset) {
        /* counters and latches are not initialised at reset */
        if (!is_reset) {
            t.latch = 0xFFFF;
            t.counter = 0;
            t.t_bit = false;
     ...
     * SR).
     * This places all peripheral interface lines in the input state, disables
    ...
    /**
     * Set a new state at the pipeline pos
     */
    ...
    /**
     * Clear a new state at the pipeline pos
     */
    ...
    private void _m6522_read_port_pins(Sim6522Input input) {
        ...
        /*
         * with latching enabled, only update the input register when CA1 / CB1 goes active
         */
    ...
    private void _m6522_tick_t1() {
        ...
        if (this.t1.t_out) {
             ...
             * continous mode
             */
            this.t1.pip = _M6522_PIP_SET(this.t1.pip, PIP_TIMER_LOAD, 1);
        }
    ...
    private void _m6522_tick_t2(Sim6522Input input) {
        ...
        if (M6522_ACR_T2_COUNT_PB6()) {
            ...
            if ((input.pb & 0x40) == 0 && (this.last_input.pb & 0x40) > 0) {
                this.t2.counter--;
            }
        ...
        if (this.t2.t_out) {
            ...
            /* NOTE: T2 never reloads from the latch on hitting zero */
        }
    }

    private void _m6522_tick_pipeline() {
        /* Feed the counter-pipelines, both counters are always counting */
    ...
    /* perform a tick (the rising edge of PHI2)*/
    ...
    /* perform a tock (the falling edge of PHI2) */
    private void update_timers(Sim6522Input input) {
        _m6522_tick_t1();
    ...
    public Sim6522Output tick(Sim6522Input input) {
         ...
         * reads to reflect the state of the previous cycle.
         * While the datasheet is not clear on this, IMHO a port read should return
         * the current state at this tick.
         */
        ...
        /* The timers are actually updated on the falling edge of phi2.
         * But to avoid introducing a seperate tock() method we update the timers at the end,
         * after reading/writing the registers.
         */
        update_timers(input);
        update_output();
